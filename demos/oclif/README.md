...
Run `pnpm --filter @clidoc/demo-oclif build`, then `pnpm --filter @clidoc/demo-oclif start greet Ada` to run the command or `pnpm --filter @clidoc/demo-oclif start __opencli` to print its OpenCLI document as JSON. The conversion reads the same metadata shape emitted into oclif's `manifest.json`.
