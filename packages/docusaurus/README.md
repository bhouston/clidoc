...
path; the plugin writes files during initialization, before the docs plugin
scans content, so the docs plugin's autogenerated sidebar discovers the pages
with no further wiring. `title` is used as each page's sidebar label. The
...
Add `outputDir` to `.gitignore`; it holds hundreds of hashed `clidoc-*.md`
files that are regenerated on every build.
