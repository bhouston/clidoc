...
export interface DocusaurusOptions {
  ...
  outputDir: string;
  basePath?: string;
  /** Docs route prefix to include in links between generated pages. */
...
export async function writeDocusaurus(
  ...
  options: { outputDir: string; basePath?: string; docsRouteBasePath?: string },
) {
  ...
  for (const [position, page] of pages.entries()) {
    ...
    const routePrefix = options.docsRouteBasePath?.replace(/^\/+|\/+$/g, '') ?? '';
    const content =
      routePrefix && page.id === 'index'
        ? page.content.replaceAll(`](${options.basePath}/commands/`, `](/${routePrefix}${options.basePath}/commands/`)
        : page.content;
