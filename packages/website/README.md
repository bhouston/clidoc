...
Browser smoke test: run `pnpm --filter @clidoc/website test:browser` after `pnpm docs:build` and installing Chromium with `pnpm --filter @clidoc/website exec playwright install chromium`.
...
The default build URL is `https://bhouston.github.io/clidoc/` for GitHub Pages. Docker sets `BASE_URL=/` for Cloud Run. Supply a `SITE_URL` Docker build argument with the eventual Cloud Run hostname for canonical links and sitemap; its fallback is the GitHub Pages host.
